    ...
    <div class="card">
        <div class="card-body">
            <div class="flex justify-between items-center mb-6">
                <h4 class="text-gray-500 text-lg font-semibold">Editar Cliente: {{ $customer->name }}</h4>
            </div>
            ...
            <form method="POST" action="{{ route('admin.customers.update', $customer) }}">
                ...
                <div class="mb-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div id="company_name_field" style="display: {{ old('type', $customer->type) === 'company' ? 'block' : 'none' }};">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            ...
                            <select name="document_type" id="document_type"
                                ...
                                <option value="">Selecione...</option>
                                <option value="cpf" {{ old('document_type', $customer->document_type) === 'cpf' ? 'selected' : '' }}>CPF</option>
                                <option value="cnpj" {{ old('document_type', $customer->document_type) === 'cnpj' ? 'selected' : '' }}>CNPJ</option>
                            </select>
                            @error('document_type')
                                <p class="text-sm text-red-500 mt-2">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="text" name="document" id="document" value="{{ old('document', $customer->document) }}"
                        ...
                        <div id="state_registration_field" style="display: {{ old('type', $customer->type) === 'company' ? 'block' : 'none' }};">
                            <label for="state_registration" class="block text-sm mb-2 text-gray-400">Inscrição Estadual</label>
                            <input type="text" name="state_registration" id="state_registration" value="{{ old('state_registration', $customer->state_registration) }}"
                ...
                <div class="mb-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="address_number" id="address_number" value="{{ old('address_number', $customer->address_number) }}"
                        ...
                        <div>
                            ...
                            <input type="text" name="complement" id="complement" value="{{ old('complement', $customer->complement) }}"
                        ...
                        <div>
                            ...
                            <input type="text" name="state" id="state" value="{{ old('state', $customer->state) }}" maxlength="2"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            ...
                            <select name="status" id="status" required
                                ...
                                <option value="">Selecione...</option>
                                <option value="active" {{ old('status', $customer->status) === 'active' ? 'selected' : '' }}>Ativo</option>
                                <option value="inactive" {{ old('status', $customer->status) === 'inactive' ? 'selected' : '' }}>Inativo</option>
                                <option value="blocked" {{ old('status', $customer->status) === 'blocked' ? 'selected' : '' }}>Bloqueado</option>
                            </select>
                            @error('status')
                                <p class="text-sm text-red-500 mt-2">{{ $message }}</p>
                            @enderror
                        ...
                        <div id="birth_date_field" style="display: {{ old('type', $customer->type) === 'individual' ? 'block' : 'none' }};">
                ...
                <div class="mb-8">
                    ...
                    <textarea name="notes" id="notes" rows="4"
                              class="py-3 px-4 text-gray-500 block w-full border-gray-200 rounded-sm text-sm focus:border-blue-600 focus:ring-0 @error('notes') border-red-500 @enderror">{{ old('notes', $customer->notes) }}</textarea>
                    @error('notes')
                        <p class="text-sm text-red-500 mt-2">{{ $message }}</p>
                    @enderror
                </div>
